...
(function($) {
	$.reject = function(opts) {
		var opts = $.extend(true,{
			...
			header: 'Ваш браузер сильно устарел!', // Header of pop-up window
			paragraph1: 'Видимо, тебе невдомек сколько проблем возникает с этим браузером при верстке и разработке веб-приложений.<br><br>\
			Сильно вдаваться в подробности не буду, однако, полезно знать, что Internet Explorer критикуется за недостаточную поддержку веб-стандартов, устанавливаемых <a href="http://ru.wikipedia.org/wiki/W3C">W3C</a>.<br><br>\
			Это означает, что браузер отображает не то, что должен отобразить согласно стандарту W3C и поэтому, может создавать проблемы веб-мастерам.<br><br>\
			Я решил облегчить себе задачу, <b>исключив IE из списка браузеров, допустимых к работе с сервисом.</b><br><br>\
			Уверен, что и твоя жизнь станет лучше, если сменишь эксплорер на более продвинутый браузер.<br><br>\
			Ниже приведен <b>список допустимых браузеров</b>.<hr>', // Paragraph 1
			paragraph2: 'Кликните по иконке браузера, чтобы скачать последнюю версию.', // Paragraph 2
			close: true, // Allow closing of window
			closeMessage: 'Закрытием этого окна, вы соглашаетесь с тем, что впечатление от использования сайта может быть испорчено.', // Message displayed below closing link
		...
		if (opts.display.length < 1) opts.display = ['chrome','msie','opera','safari','firefox'];
